...
namespace CNX.UserService.Business.Classes
{
    public class UserBusiness : BaseBusiness, IUserBusiness
    {
        ...
        public async Task<LoginResult> Login(LoginDto login)
        {
            ...
            if (user == null)
            {
                Notify("User not registered, please create an account to use the application.");
                return null;
            }
        ...
        public User GetbyCpf(string cpf) => _userRepository.GetByCpf(cpf);

        public object ValidateJwt(User user)
        {
            if (user == null)
            {
                Notify("User not valid, please login into the application.");
                return null;
            }
            return user;
        }
